...
export function createLogSetupEmbed(settings?: GuildLogSettings) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      'Mit diesem Setup kannst du die Logging-Funktionen und Berechtigungen für den Voice-Bot konfigurieren. Wähle unten eine Seite aus, um mit der Einrichtung zu beginnen.',
    )
    .setColor('#5865F2');
  ...
  if (settings) {
    embed.addFields(
      ...
      },
      {
        name: 'Mute Log',
        value:
          `Status: ${settings.mute_log_enabled ? '✅ Aktiv' : '❌ Inaktiv'}\n` +
...
export function createLogFilterButtons(filters: LogFilter) {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder()
      ...
      .setStyle(
        filters.channel_creations || filters.channel_deletions
          ? ButtonStyle.Success
    ...
    new ButtonBuilder()
      ...
      .setStyle(
        filters.auto_owner_transfers || filters.manual_owner_transfers
          ? ButtonStyle.Success
          : ButtonStyle.Secondary,
      ),
  );
